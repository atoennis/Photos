...
struct PhotoDetailViewModelTests {
    ...
    @Test func recoverFromError() async {
        var mockUseCase = MockPhotoUseCase(throwError: true)
        let useCases = MockUseCases(photoUseCase: mockUseCase)
        let viewModel = PhotoDetailViewModel(
            photoId: "0",
            useCases: useCases
        ...
        mockUseCase.throwError = false
        mockUseCase.photo = Photo.fixture(id: "0")
        viewModel.useCases = MockUseCases(photoUseCase: mockUseCase)
